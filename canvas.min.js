/*
 Canvas http://shidaixin.com/ 
 Copyright 2015 长春新时代电子科技有限公司
*/
...
var idAllocf = function (){
  var index = 0;
  var dict = {};
  return {
    alloc: function (string) {
...
function val2pos(num) {
  if (num < 0.00001)
    return num * 100000;
  scNo = num.toExponential();
  parts = scNo.split('e');
  dig = (parseFloat(parts[0])-1)/10;
  pwr = parseInt(parts[1]) + 6;
  return pwr + dig;
}
function pos2val(pos) {
  ...
  scNo = pos.toString();
  parts = scNo.split('.');
  dig = parseFloat('0.'+parts[1])*10+1;
  pwr = parseInt(parts[0]) - 6;
  return dig * Math.pow(10,pwr)
}
function str2ascii(string){
  asciiKeys = [];
  for (var i = 0; i < string.length; i ++)
    asciiKeys.push(string[i].charCodeAt(0).toString());
  var ret = asciiKeys.join('');
  return parseInt(ret);
}
  ...
  for (var i = 0, f; f = files[i]; i++) {
    ...
    reader.onload = function(e) {
      ...
      for(line in lines) {
        ...
        //console.log("from:"+divid[0]+"to:"+divid[1]);
        /*
        iBar = {
          index: words[0],
          plat:  words[2],
...
*/
      ...
      sum = idAlloc.alloc('独一无二独一ewrw无二ifjw9034ur98uf09sfik09w');
      glbl.stripLines = []
      for(i=0.5; i<sum; i++){
      ...
      drawnow1();
      //sum = idAlloc.alloc('独一无二')
      //totAll = idAlloc.alloc('独一无二ifjw9034ur98uf09sfik09w-') - 1;
      //console.log();
    }
    reader.readAsText(f);
  }
